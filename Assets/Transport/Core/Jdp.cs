...
namespace Transport
{
    internal sealed class Jdp
    {
        ...
        private const int INTERVAL = 100;           // 内部处理时钟的间隔时间
        ...
        {
            if (Utils.Sub(serialNumber, unAcknowledge) < 0 || Utils.Sub(serialNumber, sendNextSegment) >= 0)
            {
                return;
            }
            ...
            for (int i = 0; i < sendBuffers.Count; ++i)
            {
                ...
                if (Utils.Sub(serialNumber, segment.serialNumber) < 0)
                {
                    break;
                }
            }
        ...
        private void ParseSegment(Segment segment)
        {
            ...
            if (Utils.Sub(serialNumber, receiveNextSegment + receiveWindowSize) >= 0 || Utils.Sub(serialNumber, receiveNextSegment) < 0)
            {
                segmentPool.Push(segment);
                return;
            }
        ...
        private void InsertSegmentInReceiveBuffer(Segment segment)
        {
            ...
            for (index = receiveBuffers.Count - 1; index >= 0; index--)
            {
                ...
                if (Utils.Sub(segment.serialNumber, receive.serialNumber) > 0)
                {
                    break;
                }
            }
        ...
        public int Input(byte[] data, int offset, int size)
        {
            ...
            while (true)
            {
                ...
                offset += Utils.Decode32U(data, offset, out uint conv);
                if (conv != conversation) return -1;
                offset += Utils.Decode8u(data, offset, out byte command);
                offset += Utils.Decode8u(data, offset, out byte fragment);
                offset += Utils.Decode16U(data, offset, out ushort windowSize);
                offset += Utils.Decode32U(data, offset, out uint timestamp);
                offset += Utils.Decode32U(data, offset, out uint serialNumber);
                offset += Utils.Decode32U(data, offset, out uint newUnAcknowledge);
                ...
                switch (command)
                {
                    case CMD_ACK: // RTT 相关的信息，并解析序列号
                        if (Utils.Sub(currentTime, timestamp) >= 0)
                        {
                            UpdateAcknowledge(Utils.Sub(currentTime, timestamp));
                        }
                        ...
                        {
                            if (Utils.Sub(serialNumber, maxAcknowledge) > 0)
                            {
                                maxAcknowledge = serialNumber;
                            }
                        }
                    ...
                    case CMD_PUSH:// 分片的序列号在接收窗口内，则进行确认并将分片添加到接收缓冲区，然后解析数据分片
                        if (Utils.Sub(serialNumber, receiveNextSegment + receiveWindowSize) < 0)
                        {
                            AcknowledgePush(serialNumber, timestamp);
                            if (Utils.Sub(serialNumber, receiveNextSegment) >= 0)
                            {
            ...
            // 根据需要进行拥塞窗口的更新
            if (Utils.Sub(unAcknowledge, previous) > 0)
            {
        ...
        private void Flush()
        {
            ...
            while (Utils.Sub(sendNextSegment, unAcknowledge + congestionWindow) < 0)
            {
                if (sendQueue.Count == 0) break;
                var segment = sendQueue.Dequeue();
                segment.conv = conversation;
            ...
            foreach (Segment segment in sendBuffers)
            {
                ...
                else if (Utils.Sub(currentTime, segment.resendTimestamp) >= 0) // 重传超时RTO
                {
                    needSend = true;
                    segment.retransmitCount++;
                    retransmitCount++;
            ...
            void ProbeUpdate()
            {
                if (remoteWindowSize == 0)
                {
                    ...
                    {
                        if (Utils.Sub(currentTime, probeTimestamp) >= 0)
                        {
        ...
        public void Update(uint currentTime)
        {
            ...
            int slap = Utils.Sub(this.currentTime, timestampFlush); //计算距离上次刷新的时间间隔
